(function (global, factory) {
    ...
    */
    const loadResources = async function (urls, {
        ...
        const proms = await Promise.all(urls.map(async url => {
            let options = {};
            ...
            return Promise.all([autoDetectType ? res.clone().text() : res.text(), autoDetectType ? res.blob().then(b => b.type.includes('text/css')) : url.endsWith('.css'), before, after, remove, acceptErrors]);
        }));
        return Promise.all(proms.map(prom => {
            // Pass-through other promises including from `loadStylesheets`
            if (!Array.isArray(prom)) {
                if (prom.results) {
                    // Our `loadExternals` from above
            ...
            const elName = isCSS ? 'style' : 'script';
    ...
    const loadExternals = function (stylesheets, {
        before: beforeDefault = null,
        after: afterDefault = null,
        remove: removeDefault = false,
        favicon: faviconDefault = false,
    ...
    exports.loadExternals = loadExternals;
